    ...
    return (
      <section className="approval-panel approval-dock" data-testid="assessment-approval-dock" aria-label="评估结果操作">
        ...
        <div className="approval-actions">
          <Button disabled={busy} type="button" variant="secondary" onClick={requestChanges}>要求重新分析</Button>
          {props.onApprove ? <Button disabled={busy || !title.trim()} type="button" onClick={() => void run(() => props.onApprove!({ ...reference!, title: title.trim() }))}>{props.verdict === "FEATURE" ? "确认是 Feature 并开始实现" : "确认是 Bug 并开始修复"}</Button> : null}
        </div>
